# visited 리스트 대신 최소 힙 자체만으로 최단거리 계산 끝난 노드 무시하는 방법
...
def dijkstra(start):
    ...
    while q:
        ...
        # 별도의 방문 테이블을 사용하지 않고도 최소 힙에서 꺼낸 노드의 거리(dist) 가 최단거리 테이블 상의 거리(distance[cur]) 보다 크면
        # 최단거리 계산이 완료된 노드임
        if distance[cur] < dist:  # 현재 노드가 이미 처리된 적이 있는 노드이면 무시
            continue
